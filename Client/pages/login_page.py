...
class LoginPage(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        self.tf_username = ft.TextField(
            ...
            border_radius=12, color=current_theme.text_main,
            bgcolor=input_bg, border_color=ft.Colors.TRANSPARENT,
            focused_border_color=current_theme.primary,
            filled=True, height=55, text_size=14,
        ...
        self.btn_login = ft.Button(
            ...
            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=12), padding=ft.Padding(0, 18, 0, 18)),
    ...
    def build_ui(self):
        ...
        login_form_content = self.build_multi_account_view() if (self.saved_accounts and not self.selected_account) else self.build_standard_login_form()
        ...
        main_column = ft.Column(
            ...
            spacing=25,
            controls=[
                ft.Container(content=login_form_content, width=self.min_width_carousel + 40 if is_mobile else 420, alignment=ft.Alignment(0, 0)),
                self.auto_slider_banner
            ]
        )

        content_wrapper = ft.Container(expand=True, alignment=ft.Alignment(0, 0), padding=ft.Padding(20, 20, 20, 20), content=main_column)
        ...
        if self.app_page.platform == ft.PagePlatform.WINDOWS:
            ...
            btn_close_windows = ft.Container(top=0, right=0, content=ft.IconButton(ft.Icons.CLOSE, icon_size=16, style=btn_style_close, width=45, height=35, on_click=handle_login_close))

        if self.app_page.platform == ft.PagePlatform.WINDOWS:
            self.content = ft.WindowDragArea(content=ft.Stack(controls=[content_wrapper, btn_close_windows], expand=True), expand=True)
        else:
        ...
        return ft.Column(
            ...
            controls=[
                ...
                ft.Button(
                    ...
                    bgcolor=current_theme.text_main,
                    style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=12), padding=ft.Padding(0, 15, 0, 15), overlay_color=ft.Colors.with_opacity(0.1, current_theme.surface_color)),
    ...
    def build_standard_login_form(self):
        ...
        if self.saved_accounts:
            btn_back = ft.IconButton(icon=ft.Icons.ARROW_BACK_IOS_NEW_ROUNDED, icon_color=current_theme.text_main, tooltip="Quay lại danh sách tài khoản", on_click=self.back_to_multi_account)
    ...
    def request_remove_account(self, e):
        ...
        show_confirm_dialog(
            page=self.app_page, title="Gỡ tài khoản",
    ...
    async def handle_login(self, e):
        ...
        try:
            ...
            res_gv = await db_client.get("/giangvien", params={"select": "*", "auth_id": f"eq.{user_id}"})
