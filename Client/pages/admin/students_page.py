...
class StudentsPage(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        # -- UI Elements --
        self.title_text = ft.Text("QUẢN LÝ SINH VIÊN", size=24, weight=ft.FontWeight.BOLD, color=current_theme.text_main)
        ...
        self.search_field = ft.TextField(hint_text="Tìm kiếm theo MSSV hoặc Tên...", icon=ft.Icons.SEARCH, height=40, expand=True, on_change=self.filter_data)
        ...
        self.data_table = ft.DataTable(
            columns=[
                ...
                ft.DataColumn(ft.Text("Họ Đệm", weight=ft.FontWeight.BOLD)),
                ft.DataColumn(ft.Text("Tên", weight=ft.FontWeight.BOLD)),
        ...
        # -- Form Dialog --
        self.form_id = ft.TextField(label="MSSV")
        self.form_ho = ft.TextField(label="Họ Đệm")
        self.form_ten = ft.TextField(label="Tên")
        self.form_lop = ft.TextField(label="Mã Lớp")
        
        self.dialog = ft.AlertDialog(
            title=ft.Text("Thông tin Sinh viên"),
            content=ft.Column([self.form_id, self.form_ho, self.form_ten, self.form_lop], tight=True),
            actions=[
    ...
    async def load_data(self):
        ...
        self.all_data = [
            {"student_id": "SV001", "hodem": "Nguyễn Văn", "ten": "A", "class_id": "L01"},
            {"student_id": "SV002", "hodem": "Trần Thị", "ten": "B", "class_id": "L02"},
            {"student_id": "SV003", "hodem": "Lê", "ten": "C", "class_id": "L01"},
        ]
    ...
    def filter_data(self, e):
        ...
        self.filtered_data = [item for item in self.all_data if query in str(item.get("student_id", "")).lower() or query in str(item.get("ten", "")).lower() or query in str(item.get("hodem", "")).lower()]
        self.current_page = 1
        self.render_table()
    ...
    def render_table(self):
        ...
        for item in page_data:
            self.data_table.rows.append(
                ft.DataRow(cells=[
                    ft.DataCell(ft.Text(item.get("student_id", ""))),
                    ft.DataCell(ft.Text(item["hodem"])),
                    ft.DataCell(ft.Text(item["ten"])),
                    ft.DataCell(ft.Text(item["class_id"])),
                    ft.DataCell(ft.Row([
    ...
    def open_edit_dialog(self, data):
        self.form_id.value = data.get("student_id", "")
        self.form_ho.value = data["hodem"]
        self.form_ten.value = data["ten"]
        self.form_lop.value = data["class_id"]
        self.app_page.overlay.append(self.dialog)
        self.dialog.open = True
        self.app_page.update()
    ...
    async def _save_data_async(self):
        self.close_dialog()
        await asyncio.sleep(0.3)
        show_top_notification(self.app_page, "Đã lưu sinh viên thành công!", ft.Colors.GREEN)
        await self.load_data()

    def delete_data(self, data):
        def on_confirm():
            self.app_page.run_task(self._delete_data_async, data["student_id"])
        show_confirm_dialog(self.app_page, "Xác nhận", f"Xóa sinh viên {data['ten']}?", on_confirm)

    async def _delete_data_async(self, id):
        await asyncio.sleep(0.3)
        show_top_notification(self.app_page, "Đã xóa sinh viên thành công!", ft.Colors.GREEN)
        await self.load_data()
