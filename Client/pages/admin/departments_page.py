...
class DepartmentsPage(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        # -- UI Elements --
        self.title_text = ft.Text("QUẢN LÝ KHOA", size=24, weight=ft.FontWeight.BOLD, color=current_theme.text_main)
        ...
        self.search_field = ft.TextField(hint_text="Tìm kiếm theo ID hoặc Tên...", icon=ft.Icons.SEARCH, height=40, expand=True, on_change=self.filter_data)
        self.page_size_dropdown = ft.Dropdown(
            options=[ft.dropdown.Option("10"), ft.dropdown.Option("20"), ft.dropdown.Option("50")],
            value="10",
            width=80,
            height=40,
            on_select=self.change_page_size,
            content_padding=5
        )

        self.data_table = ft.DataTable(
            columns=[
                ft.DataColumn(ft.Text("Mã Khoa (ID)", weight=ft.FontWeight.BOLD)),
                ft.DataColumn(ft.Text("Mã Phụ", weight=ft.FontWeight.BOLD)),
                ft.DataColumn(ft.Text("Tên Khoa", weight=ft.FontWeight.BOLD)),
        ...
        # -- Form Dialog --
        self.form_id = ft.TextField(label="Mã Khoa (ID)")
        self.form_code = ft.TextField(label="Mã Phụ (Code)")
        self.form_name = ft.TextField(label="Tên Khoa")
        self.form_desc = ft.TextField(label="Mô tả", multiline=True)
        
        self.dialog = ft.AlertDialog(
            title=ft.Text("Thông tin Khoa"),
            content=ft.Column([self.form_id, self.form_code, self.form_name, self.form_desc], tight=True),
            actions=[
    ...
    async def load_data(self):
        ...
        self.all_data = [
            {"id": "K01", "code": "CNTT", "tenkhoa": "Công nghệ Thông tin"},
            {"id": "K02", "code": "KT", "tenkhoa": "Kinh tế"},
            {"id": "K03", "code": "NN", "tenkhoa": "Ngoại ngữ"},
            {"id": "K04", "code": "DU", "tenkhoa": "Dược"},
        ]
    ...
    def filter_data(self, e):
        ...
        self.filtered_data = [item for item in self.all_data if query in item["id"].lower() or query in item["tenkhoa"].lower()]
        self.current_page = 1
        self.render_table()
        ...
        for item in page_data:
            self.data_table.rows.append(
                ft.DataRow(cells=[
                    ...
                    ft.DataCell(ft.Text(item["tenkhoa"])),
                    ft.DataCell(ft.Row([
    ...
    def open_edit_dialog(self, data):
        ...
        self.form_name.value = data["tenkhoa"]
        self.form_desc.value = data.get("description", "")
        self.app_page.overlay.append(self.dialog)
        self.dialog.open = True
        self.app_page.update()
    ...
    async def _save_data_async(self):
        self.close_dialog()
        await asyncio.sleep(0.3)
        show_top_notification(self.app_page, "Đã lưu khoa thành công!", ft.Colors.GREEN)
