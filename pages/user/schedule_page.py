...
class SchedulePage(ft.Container):
    ...
    async def _async_rebuild_entire_ui(self):
        ...
        if getattr(self, "app_page", None): self.app_page.update()
        self.is_updating_ui = False
    ...
    def _set_selected_date_and_scroll(self, target_date: datetime.date):
        ...
        if old_ref:
            old_ref["bg"].bgcolor = current_theme.surface_color if self.view_mode == "week" else ft.Colors.TRANSPARENT
            old_ref["text"].color = current_theme.text_main
            old_ref["text"].weight = ft.FontWeight.NORMAL if self.view_mode == "month" else ft.FontWeight.W_800
            if "day_text" in old_ref: old_ref["day_text"].color = current_theme.text_muted
            old_ref["bg"].update()

        self.selected_date = target_date
        new_ref = self.calendar_day_refs.get(self.selected_date)
        if new_ref:
            new_ref["bg"].bgcolor = current_theme.secondary
            new_ref["text"].color = current_theme.bg_color
            new_ref["text"].weight = ft.FontWeight.W_500 if self.view_mode == "month" else ft.FontWeight.W_800
            if "day_text" in new_ref: new_ref["day_text"].color = current_theme.bg_color
            new_ref["bg"].update()
    ...
    def render_toolbar(self):
        def create_tab(text, value):
            ...
            return ft.Container(
                content=ft.Text(text, color=current_theme.bg_color if active else current_theme.text_muted, weight=ft.FontWeight.BOLD if active else ft.FontWeight.NORMAL, size=12),
                bgcolor=current_theme.secondary if active else ft.Colors.TRANSPARENT,
                padding=ft.Padding(15, 6, 15, 6), border_radius=8, ink=True, data=value, on_click=self._handle_custom_mode_change,
            )

        segment_group = ft.Container(
            bgcolor=current_theme.surface_color, border=ft.Border.all(1, current_theme.divider_color), border_radius=8, padding=3,
            content=ft.Row(spacing=0, controls=[create_tab("Ngày", "day"), create_tab("Tuần", "week"), create_tab("Tháng", "month")]),
        )
        ...
        if self.view_mode == "day":
            ...
            dd_week = ft.Dropdown(value=current_week_id, options=week_opts, text_size=14, border=ft.InputBorder.NONE, dense=True, width=120, menu_height=250, color=current_theme.text_main)
        ...
        if self.view_mode == "week":
            ...
            for i in range(7):
                ...
                box = ft.Container(
                    ...
                    border_radius=12, padding=ft.Padding(0, 10, 0, 10), border=ft.Border.all(1, current_theme.divider_color),
