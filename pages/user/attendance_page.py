...
class AttendancePage(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        # ==========================================
        # KHỞI TẠO CÁC CONTROL TĨNH (Không tạo lại để giữ State)
        # ==========================================
        ...
        # ==========================================
        # CÁC DIALOG VÀ BOTTOM SHEET (Lớp phủ)
        # ==========================================
    ...
    def apply_theme(self):
        """Hàm vẽ lại cấu trúc Layout khi Theme thay đổi"""
        ...
        # Cập nhật màu cho lớp phủ Dialog
    ...
    def build_main_layout(self):
        ...
        combined_attendance_card = make_card(
            ft.Column([
                ...
                # Đặt trực tiếp list_view vào Column thay vì wrap Container thêm
    ...
    async def load_all_schedules(self):
        ...
        try:
            ...
            self.build_schedule_cards()
        except Exception as e: print(f"Lỗi fetch: {e}")
    ...
    def build_schedule_cards(self):
        ...
        for tkb in self.tkb_data:
            ...
            btn_confirm = ft.Button(content=ft.Text("Xác nhận", size=12, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE), bgcolor=current_theme.accent, height=40, style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=6)), on_click=create_select_handler(tkb, local_dd_date))
    ...
    async def handle_test_camera(self, e):
        ...
        async def run_test():
            ...
            show_top_notification(
                self.app_page, 
                "AuEdu", 
                "Camera hoạt động tốt!" if is_ok else "Opps! Có vẻ camera đang lỗi!", 
                color=ft.Colors.GREEN if is_ok else ft.Colors.ORANGE,
                sound= "S" if is_ok else "E"
                )
